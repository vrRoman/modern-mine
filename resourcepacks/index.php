...
<html lang="ru">
  <head>
    ...
    <link rel="stylesheet" href="styles/resourcepacks.min.css">
  ...
  <body>
      ...
      <div class="form">
        ...
        <form action="" method="get">
          <div class="form__search">
            ...
            <input id="search" name="search" type="search" placeholder="Название ресурспака">
          </div>
          ...
          <div class="form__categories">
            <span>Категории:</span>
            <label class="any" for="any-category">Любая</label>
            <input id="any-category" type="checkbox" value="any" name="categories[]">

            <label for="pvp-category">ПВП</label>
            <input class="categories-inputs" id="pvp-category" type="checkbox" value="pvp"
                   name="categories[]">
            <label for="fps-boost-category">ФПС Буст</label>
            <input class="categories-inputs" id="fps-boost-category" type="checkbox" value="fps-boost"
                   name="categories[]">
            <label for="bw-category">БВ</label>
            <input class="categories-inputs" id="bw-category" type="checkbox" value="bw"
                   name="categories[]">
            <label for="beautiful-category">Красивые</label>
            <input class="categories-inputs" id="beautiful-category" type="checkbox" value="beautiful"
                   name="categories[]">
          </div>
    ...
    <script>
      inputsSwitch(document.getElementById('any-category'), document.querySelectorAll('.categories-inputs'));
...
<?php
  ...
  $query = "SELECT * FROM `resourcepacks`";
    ...
    if (!trim($_GET['search'])) {
      foreach ($categories as $key => $category) {
        if ($categoriesCount == 1) {
          ...
          $query .= " WHERE (`tags` LIKE '%$category%'";
        } elseif ($key == $categoriesCount - 1) {
          $query .= " OR `tags` LIKE '%$category%')";
        } else {
          $query .= " OR `tags` LIKE '%$category%'";
        }
      }
  ...
  if ($versions != 'any') {
    ...
    if (!trim($_GET['search']) && $categories == 'any') {
  ...
  foreach ($allItems as $item) {
    echo "<script>
      Render.itemPreview(document.querySelector('.items'),
        '$item[1]', '$item[3]', '$item[4]', $item[6], $item[9], {
          rating: $item[11],
          imgUrlNotActive: '../img/star.svg',
          imgUrlActive: '../img/active-star.svg'
        }, $item[0], 'items__item', '[$item[8]x]')
    </script>";
  }
