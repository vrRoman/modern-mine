<?php
  ...
  if (isset($data['change'])) {
    $itemId = $data['itemId'];
    $currentRating = mysqli_fetch_assoc(mysqli_query($connect, "SELECT `rating` FROM `mods` WHERE `id` = '$itemId'"))['rating'];
    $numOfRatings = mysqli_fetch_assoc(mysqli_query($connect, "SELECT `num_of_ratings` FROM `mods` WHERE `id` = '$itemId'"))['num_of_ratings'];
    ...
    if ($data['change']) {
      ...
      $query = "UPDATE `mods` SET `rating` = '$newItemRating' WHERE `id` = '$itemId'";
